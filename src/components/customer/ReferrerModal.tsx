...
// Kiểu dữ liệu cho người giới thiệu, dùng '?' để cho phép giá trị null
interface Referrer {
    id?: string;
    name?: string;
    phone?: string;
...
export function ReferrerModal({ open, onClose }: ReferrerModalProps) {
    const [selectedReferrerId, setSelectedReferrerId] = useState('EM0127');
    // State để lưu thông tin form, khởi tạo từ người được chọn mặc định
    ...
    const handleSelectReferrer = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFormData(selectedData); // Tự động điền dữ liệu vào form
    };
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            <DialogTitle>Người giới thiệu</DialogTitle>
            <DialogContent sx={{ bgcolor: grey[50], p: 2 }}>
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={9}>
                        <Box sx={{ p: 2, bgcolor: 'white', borderRadius: 2, border: `1px solid ${grey[200]}` }}>
                            {/* Phần tìm kiếm */}
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} md={6}><FormControl fullWidth variant="filled" sx={inputStyles}><InputLabel shrink>Chi nhánh</InputLabel><Select value={formData?.branch || 'q1'}><MenuItem value="q1">Nha khoa Cẩm Tú - Quận 1</MenuItem></Select></FormControl></Grid>
            ...
            <DialogActions sx={{ p: '16px 24px' }}>
                <Button variant="outlined" sx={{ textTransform: 'none', borderRadius: '8px' }} onClick={onClose}>Đóng</Button>
